...
当前版本不提供用户可见的账户注册，也不要求使用电子邮件地址、密码或第三方账号登录。
为提供云端功能，应用会自动创建并处理 Firebase 匿名用户 ID。我们使用这一假名标识符进行
认证、配额执行、防欺诈和防滥用，以及购买权益验证。如未来引入用户可见的账户功能，
我们会在启用前更新本政策。
...
使用内置模型服务时，输入文本、所选语境包指令及生成响应所需的信息会经由 Firebase 后端
发送至 Google Gemini。我们的后端不会将普通转换文本或生成结果作为长期内容记录保存。

如果你选择 OpenAI 兼容的自定义模型服务，应用会将输入文本、所选语境包指令和生成请求
从你的设备直接发送至你配置的 HTTPS 端点。API Key 仅会发送给该服务提供方以验证请求，
不会发送至我们的 Firebase 后端。该服务提供方对数据的处理、保留、处理地点和收费，
均受其自身条款和隐私政策约束。

使用语音输入时，录音会发送至我们的 Firebase 后端和 Google Gemini 以生成转写文本。
转写操作完成后，我们的后端不会保存录音、转写文本或服务提供方响应。Google Gemini 对录音的
处理受其适用的条款和隐私实践约束。
...
导入的自定义语境包和自定义模型服务配置也保存在设备本地。自定义模型服务的 API Key
使用平台安全存储保存，不会写入普通应用偏好设置。
{% endif %}
...
为了执行使用限制和订阅权益，我们会处理匿名用户 ID、配额余额、续期或重置周期、请求次数、
...
当你联系支持时，我们会处理你主动提供的联系方式、消息、附件和技术信息，以便调查和回复。

如果你提交不当输出举报，我们会处理并保存生成结果、举报类别、可选备注和有限的生成上下文。
你也可以选择随举报一并提供原始输入文本。这些记录仅用于安全审核、调查和改进，
不会因普通转换而创建。
...
- **Google Firebase** — {{ app.privacy.firebase_services }}；Firebase Authentication 提供匿名认证，
  App Check 协助保护服务，Cloud Functions 处理云端请求，Firestore 存储本政策所述记录；
- **Google Play** — 应用分发、订阅账单、购买记录和权益信息；
- **Google Gemini** — 提供内置模型服务的生成和语音转写。请参阅
  [Gemini API 附加服务条款](https://ai.google.dev/gemini-api/terms) 和
  [Google 隐私权政策](https://policies.google.com/privacy)，了解 Google 适用的数据处理条款；
- 你选择的自定义模型服务提供方 — 当你配置并使用该服务时，直接处理生成请求；
...
- 导入的语境包、自定义模型服务配置及本地保存的 API Key 会保留在设备上，直到你移除它们、
  清除应用数据或卸载应用；
- 已完成的使用事件和配额账本会在相关事件或配额周期结束后最多保留 90 天，其中不包含普通
  转换文本或生成结果；
- 不当输出举报及你选择随举报提交的内容会保留 90 天；
- 购买和权益记录会在提供和验证有效权益所需的期间内保留。权益结束后，相关记录通常会因
  防欺诈、争议处理、会计和法律合规而最多保留 24 个月。有效的终身权益会在其持续有效期间
  保留，以便验证或撤销权益；
...
我们采用合理的管理、技术和组织措施，包括经认证的后端访问、受限的服务凭据、传输加密和
仅允许 HTTPS 的自定义模型服务端点。自定义模型服务的 API Key 使用设备的平台安全存储保存。
...
由于应用不提供用户可见的账户，在处理请求前，我们可能需要你提供有助于定位相关服务端记录的
信息。我们会依照适用法律处理隐私请求。

## 9. 儿童隐私

{{ app.name }} 不面向、也不意图向未满 18 周岁的人提供服务。我们不会故意向未满 18 周岁的人
提供本服务。如你认为未满 18 周岁的人通过本服务提供了个人信息，请联系我们进行审查。
